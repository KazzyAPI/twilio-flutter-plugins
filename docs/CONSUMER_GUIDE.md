...
This page is for app developers who add **`twilio_flutter_conversations`** and/or **`twilio_flutter_video`** from pub.dev. Each package is published separately; you do not install the Git monorepo as a single dependency.
...
| Core | `twilio_flutter_core` | Pulled in automatically; you rarely depend on it directly |
...
When these packages are on pub.dev, declare them in your app’s `pubspec.yaml`:
...
dependencies:
  ...
  # Chat (Twilio Conversations classic)
  twilio_flutter_conversations: ^0.1.0

  # Optional: Programmable Video in the same app
