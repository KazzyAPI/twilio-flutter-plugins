...
| `twilio_flutter_video` | Programmable Video bridge (connect, events, local track toggles). |
...
Same pattern as Conversations: `TwilioVideoSession` → `TwilioVideoClient` → Pigeon → `VideoBridge` → Twilio Video SDK. Spec: `specs/002-twilio-flutter-video/`.
