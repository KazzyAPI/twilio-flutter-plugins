...
Generated Flutter APIs use **suspend**/`async` on Android and Swift. Native event emitters must call `onNativeEvent` from a coroutine (Android) or `Task { @MainActor in … }` (iOS), not synchronously from Twilio SDK callback threads. On iOS, callback-based bridge code sits behind `ConversationsBridgeHostAdapter` (same pattern as Android). Conversations Android **6.2.1** moved several types (for example `com.twilio.util.ErrorInfo`, updated sync/connection enums); keep bridges aligned with the pinned SDK Javadoc when CI Kotlin compile fails after a bump.
